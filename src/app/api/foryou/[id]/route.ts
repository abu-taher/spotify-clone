...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const { id: forYouId } = params;

    // Fetch album details including artist information
    const albumResponse = await fetch(`${process.env.API_BASE_URL}/radio/${forYouId}/tracks`);
    if (!albumResponse.ok) {
    ...
    // Extract album and artist details from the first track (assuming all tracks are from the same album and artist)
    ...
    const formattedTracks = data.map((track: { id: string; title: string; duration: number; preview: string }) => ({
      ...
      album:  firstTrack.album.title || 'Unknown',
